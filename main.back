...
CONFIG = {
    ...
    "USE_PROXY": False,  # 是否启用代理（GitHub Actions环境建议设为False）
        ...
        try:
            ...
            for stat in stats:
                for title_data in stat.get("titles", []):
                    all_news_titles.append(title_data.get("title", ""))
            
            # 收集新增新闻标题
            if new_titles:
                for titles_data in new_titles.values():
                    if isinstance(titles_data, dict):
                        all_news_titles.extend(titles_data.keys())
            
            etf_strategy_results = etf_analyzer.analyze_strategy(all_news_titles)
            report_data["etf_strategy"] = etf_strategy_results
            print("✅ HTML报告ETF策略分析完成")
        except Exception as e:
            print(f"⚠️ HTML报告ETF策略分析失败: {e}")
            report_data["etf_strategy"] = {}
    ...
    def _render_html_content(
        ...
        if report_data.get("etf_strategy"):
            ...
            for index_name, strategy_result in report_data["etf_strategy"].items():
                escaped_index = ReportGenerator._html_escape(index_name)
                # 处理策略结果中的markdown格式，转换为HTML
                escaped_result = ReportGenerator._html_escape(strategy_result)
                escaped_result = escaped_result.replace("**", "<strong>").replace("**", "</strong>")
                escaped_result = escaped_result.replace("\n", "<br>")
